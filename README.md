...
This is a customized Version of the WebUi from the ESP3D Project from Luc. I have changed the Design for a better optic and added a note-function called "Safe-Memo". This Note Function works with localStorage, that means it works only on one PC and loose the data if some other device is used. That's because the localStorage is written on every device. So keep it in mind - Your data are not loosed, you must go to the device where you have added the Note in SafeMemo-Tab.
