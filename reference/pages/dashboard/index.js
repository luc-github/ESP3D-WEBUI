    ...
    useEffect(() => {
        if (!panels.initDone && panels.list.length != 0) {
            if (isfixed && fixedPanels.length == 0) {
                ...
                if (!Array.isArray(panelOrder)) return
                let orderIndex = 0
                    ...
                    const id =
                        nameField != null && nameField.value != null
                            ...
                            : (panel.value &&
                              ...
                              panel.id
                    if (id === "extracontents") {
                        const extraPanels = panels.list.filter((p) => (p.settingid || "").startsWith("extracontents_"))
                        extraPanels.forEach((p) => {
                            fixedPanels.push({ index: orderIndex++, id: p.settingid })
                        })
                    } else {
                ...
                // Persist expanded order (extracontents -> extracontents_xxx) so save/export uses individual panels
                const next = JSON.parse(JSON.stringify(uisettings.current))
                ...
                if (el && Array.isArray(el.value)) {
                    ...
                    reordered.forEach((item) => {
                        if (item.value && item.value[0]) item.value[0].hasmodified = true
                    })
                    uisettings.set(next)
                    if (interfaceSettings.current) interfaceSettings.current.settings = next
                }
            } else {
                panels.setVisibles(
        ...
        if (uisettings.getValue("showextracontents")) {
            ...
            const extraPanelsList = extraContents.reduce((acc, curr) => {
                ...
                if (item.target == "panel") {
                    const el = ExtraPanelElement(item, curr.id)
                    acc.push(el)
                }
                return acc
            }, [])
        ...
        <div id="dashboard">
            ...
            >
                {panels.visibles.map((panel, index) => {
                    ...
                    const isExtraPanel = panel.settingid && panel.settingid.startsWith("extracontents_")
                    ...
                    const handleDrop = (e) => {
                        ...
                        if (el && Array.isArray(el.value)) {
                            ...
                            reordered.forEach((item, i) => {
                                ...
                                const idxField =
                                    item.value &&
                                    item.value.find((s) => s.name === "index")
