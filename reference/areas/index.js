...
const ContentContainer = () => {
    ...
    const processExtensionMessage = (eventMsg) => {
        if (eventMsg.data.type && eventMsg.data.target == "webui") {
            const reply = (type, data, init, id) => {
                if (eventMsg.data.noDispatch) return
                dispatchToExtensions(type, { ...data, initiator: init ?? eventMsg.data }, id ?? eventMsg.data.id)
            }
            switch (eventMsg.data.type) {
                ...
                case "cmd":
                    createNewRequest(
                        espHttpURL("command", { cmd: eventMsg.data.content }),
                        { method: "GET" },
                        {
                            onSuccess: (result) => reply("cmd", { status: "success", response: result }),
                            onFail: (err) => reply("cmd", { status: "error", error: err }),
                        }
                    )
                    break
                case "query":
                    const cmd = eventMsg.data.url === "command" ? eventMsg.data.args.cmd : null
                    createNewRequest(
                        espHttpURL(eventMsg.data.url, eventMsg.data.args),
                        { method: "GET", echo: cmd },
                        {
                            onSuccess: (result) => reply("query", { status: "success", response: result }),
                            onFail: (err) => reply("query", { status: "error", error: err }),
                        }
                    )
                    break
                case "upload": {
                    const formData = new FormData()
                    const file = new File([eventMsg.data.content], eventMsg.data.filename)
                    const initiator = {
                        type: "upload", id: eventMsg.data.id, url: eventMsg.data.url,
                        target: eventMsg.data.target, path: eventMsg.data.path,
                        filename: eventMsg.data.filename, size: eventMsg.data.size,
                        args: eventMsg.data.args, noDispatch: eventMsg.data.noDispatch,
                    }
                    ...
                    createNewRequest(
                        ...
                        {
                            onSuccess: (result) => reply("upload", { status: "success", response: result }, initiator),
                            onFail: (err) => reply("upload", { status: "error", error: err }, initiator),
                            onProgress: (e) => reply("upload", { status: "progress", progress: e }, initiator),
                        }
                    )
                    break
                }
                case "download":
                    createNewRequest(
                        ...
                        {
                            onSuccess: (result) => reply("download", { status: "success", response: result }),
                            onFail: (err) => reply("download", { status: "error", error: err }),
                            onProgress: (e) => reply("download", { status: "progress", progress: e }),
                        }
                    )
                    break
                case "toast":
                ...
                case "extensionsData":
                    ...
                    //now update the settings section with export version (and omit defaults to reduce size)
                    const fullSection = exportPreferencesSection(interfaceSettingsData.settings, false, true)
                    const defaultSettings = exportPreferencesSection(defaultPreferences.settings, true)
                    interfaceSettingsData.settings = omitDefaultSettings(fullSection, defaultSettings)

                    //now stringify and save
                    const preferencestosave = JSON.stringify(interfaceSettingsData)
                    //Create a blob
